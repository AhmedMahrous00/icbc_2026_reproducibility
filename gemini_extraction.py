...
def call_gemini(prompt, model="gemini-3.1-pro-preview", temperature=0.0, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                print(f"Error calling Gemini API after {max_retries} attempts: {str(e)}")
                return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract coin and exchange information from pump messages using Gemini API.")
    ...
    output_filename = args.output_filename or "gemini_extraction.csv"
    if args.test_run_pumps is not None and args.test_run_pumps > 0 and args.output_filename is None:
        output_filename = "gemini_extraction_test.csv"
    ...
    for p_idx in tqdm(pump_idxs, desc="Processing pump events"):
        ...
        response = call_gemini(prompt, model=args.model, temperature=args.temperature)
